-- Minimal SQL script to create quiz tables in your Supabase database
-- This version uses the simplest RLS policies that should work regardless of other tables
...
-- Simple RLS policies for quiz tables - these use basic auth.uid() checks

-- Policy for quizzes table: users can see active quizzes
CREATE POLICY "Users can view active quizzes" ON quizzes
FOR SELECT TO authenticated USING (
  is_active = true
);

-- Policy for quiz_questions table: users can see questions
CREATE POLICY "Users can view quiz questions" ON quiz_questions
FOR SELECT TO authenticated USING (true);

-- Policy for quiz_options table: users can see options
